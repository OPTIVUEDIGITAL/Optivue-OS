...
function primaryBottleneck(statuses, a) {
  ...
  if (statuses.Acquisition === 'High Priority') return {title:'Your acquisition plan needs a clearer demand strategy.',why:'The current channel mix does not yet provide a consistent path for bringing qualified prospects into the system.',action:'Choose the highest-intent acquisition channels and connect them to a measurable conversion path.'};
  return {title:'Your foundation is in place. The next opportunity is optimization.',why:'The reported systems cover the core journey, so improvement should focus on friction, attribution, and performance across the connected funnel.',action:'Review conversion data, pipeline quality, and channel efficiency to identify the next highest-priority improvement.'};
}
