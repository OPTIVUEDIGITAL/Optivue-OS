...
export function getNotedInsight(answers = {}, previousAnswers = {}, insightCount = 0) {
  ...
  const candidates = [
    [countUnsure(answers) >= 3 && countUnsure(previousAnswers) < 3, 'Not knowing is useful information too. It usually means a review should come before spending more.'],
    [answers.follow_up_process === 'depends' && previousAnswers.follow_up_process !== 'depends', "Noted. When follow-up depends on who's around, some leads wait much longer than others. I'll factor that in."],
    [answers.response_speed === 'next_day' && previousAnswers.response_speed !== 'next_day', 'Noted. Reply speed is one of the easiest gaps to check and fix.'],
    [answers.primary_problem === 'cant_tell' && previousAnswers.primary_problem !== 'cant_tell', 'Noted. That usually points to tracking, not the marketing itself.'],
    [answers.current_systems?.includes('none') && !previousAnswers.current_systems?.includes?.('none'), "That's fine. Many strong businesses start here. It just changes what comes first."],
  ];
...
export function initEstimator(root) {
  ...
  function renderIntro() {
    mount.innerHTML = `<div class="ovgo-estimator-intro"><p class="ovgo-kicker">Growth System Scope Estimator</p><h2>See where your leads slip away, and what fixing it involves.</h2><p>Answer 9 quick questions about how inquiries reach you and what happens next. You'll get a snapshot of your growth system, a recommended next step, and a realistic investment range.</p><button class="ovgo-btn ovgo-btn--primary" type="button" data-estimator-start>Start the Estimate</button><ul class="ovgo-estimator-facts"><li>About 90 seconds</li><li>No email needed to see results</li><li>Not a fixed quote</li></ul><p class="ovgo-form-note">Please don't enter patient information or passwords anywhere in this tool.</p></div>`;
